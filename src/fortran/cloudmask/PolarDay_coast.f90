      subroutine PolarDay_coast(pxldat,vza,visusd,cirrus_vis,hi_elev,  &
...
!
!!Description:
!      Routine for performing clear sky tests over polar coastal  
!      surfaces during daylight hours.
!
!      For Polar day coast the groups are:
...
! ... local scalars ..
      real c1,c2,c3,c4,c5,cosvza,dfthrsh,diftemp,dtr,mas11_4,masdf1,    &
           masir11,masir12,masir4,masv188,masv66,pi,schi,            &
...
!      masir12 = pxldat(32)
      ...
      masv66 = pxldat(3)
      masv188 = pxldat(19)
      masir4 = pxldat(21)  ! 4.05 replace modis 3.959
!      masir65 = pxldat(27)
      masir11 = pxldat(24)
      masir12 = pxldat(25)
...
! ... debug statement ............................................
!      if (debug .gt. 0) then
!        write(h_output,'(10x/,''Processing subroutine PolarDay_coast '',
!     +                      /)') 
!      endif
! ................................................................
...
!        nmtests = nmtests + 1
!        call set_qa_bit(qa_bits,15)
!        if (masir65 .gt. pdlh20_t2(2)) then
!          call set_bit(testbits,15)
!          nptests = nptests + 1
!        end if
!        call conf_test(masir65,pdlh20_t2(1),pdlh20_t2(3),pdlh20_t2(4),
!     *                pdlh20_t2(2),1,c2)
!        cmin1 = min(cmin1,c2)
!        ngtests(1) = ngtests(1) + 1
!      endif
...
! ... debug statement ............................................
!      if (debug .gt. 0) then
!        write(h_output,'(1x,''masir65: '',5f10.2)') masir65,pdlh20_t2(1),
!     +          pdlh20_t2(2),pdlh20_t2(3),pdlh20_t2(4)
!      endif
! ................................................................
...
! ... for now placing in the SWIR bit place holder
      if (visusd) then
        if (nint(masir11) .ne. nint(bad_data) .and.   &
          ...
          if (mas11_4 .ge. pdl11_4lo_t2(2)) then
            call set_bit(testbits,19)
            nptests = nptests + 1
          end if
          call conf_test(mas11_4,pdl11_4lo_t2(1),pdl11_4lo_t2(3),pdl11_4lo_t2(4),   &
                         pdl11_4lo_t2(2),1,c4)
          cmin2 = min(cmin2,c4)
          ngtests(2) = ngtests(2) + 1
        endif
...
! ... debug statement ............................................
!      if (debug .gt. 0) then
!         write(h_output,'(1x,''mas11_4: '',5f10.2)')mas11_4,pdl11_4lo_t2(1),
!     +            pdl11_4lo_t2(2),pdl11_4lo_t2(3),pdl11_4lo_t2(4)
!      endif
! ................................................................
      end if
...
! ... visible (channel 1) reflectance threshold test.
      if (visusd) then
        if (nint(masv66) .ne. nint(bad_data)) then 
          ...
          if (masv66 .le. pdlref1_t2(2)) then
            call set_bit(testbits,20)
            nptests = nptests + 1
          end if
          call conf_test(masv66,pdlref1_t2(1),pdlref1_t2(3),pdlref1_t2(4),   &
                         pdlref1_t2(2),1,c5)
          cmin3 = min(cmin3,c5)
...
! ...   debug statement ............................................
!        if (debug .gt. 0) then
!          write(h_output,'(1x,''masv66: '',5f10.2)') masv66,pdlref1_t2(1),
!     +            pdlref1_t2(2),pdlref1_t2(3),pdlref1_t2(4)
!        endif
! ................................................................
      end if
...
! ... near infrared high cloud test
      if (visusd .and. (.not. hi_elev) ) then
        if (nint(masv188) .ne. nint(bad_data)) then 
          ...
          if (masv188 .le. pdlref3_t2(2)) then
            call set_bit(testbits,16)
            nptests = nptests + 1
          end if
          call conf_test(masv188,pdlref3_t2(1),pdlref3_t2(3),pdlref3_t2(4),   &
                         pdlref3_t2(2),1,c7)
          cmin4 = min(cmin4,c7)
...
! ...   debug statement ............................................
!        if (debug .gt. 0) then
!           write(h_output,'(1x,''masv188: '',6f10.4)')masv188,pdlref3_t2(1),
!     +                pdlref3_t2(2),pdlref3_t2(3),pdlref3_t2(4)
!        endif
! ................................................................
      end if
...
!     Check to see if thin cirrus bit should be set
      if (visusd .and. (.not. hi_elev) ) then
        if (nint(masv188) .ne. nint(bad_data)) then 
          ...
          if (masv188 .lt. pdltci_t2(1) .and. masv188 .ge. pdltci_t2(2)) then
