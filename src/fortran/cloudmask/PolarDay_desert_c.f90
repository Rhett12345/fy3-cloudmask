...
!
!!Description:
!      Routine for performing clear sky tests over polar coastal desert
!      surfaces during daylight hours.
!
!      For daytime polar coastal desert the groups are:
...
!----------------------------------------------------------------------
      if (nint(masir11) .ne. nint(bad_data) .and.  &
        ...
        if (masdf1.le.pds11_12hi_c(2)) then
          nmtests = nmtests + 1
          call set_bit(testbits,18)
          nptests = nptests + 1
          ngtests(1) = ngtests(1) + 1
        end if
        call conf_test(masdf1,pds11_12hi_c(1),pds11_12hi_c(3),   &
                       pds11_12hi_c(4),pds11_12hi_c(2),1,c1)
        cmin1 = min(cmin1,c1)
      endif
...
!----------------------------------------------------------------------
      if (visusd) then
        if (nint(masir11) .ne. nint(bad_data) .and.   &
          ...
          if (mas11_4.ge.pds11_4lo_c(2)) then
            nmtests = nmtests + 1
          ...
          call conf_test(mas11_4,pds11_4lo_c(1),pds11_4lo_c(3),   &
                         pds11_4lo_c(4),pds11_4lo_c(2),1,c2)
          call conf_test(mas11_4,pds11_4hi_c(1),pds11_4hi_c(3),   &
                         pds11_4hi_c(4),pds11_4hi_c(2),1,c3)
...
!       0.86um test
        if (nint(masv88) .ne. nint(bad_data)) then
          ...
          if (masv88 .le. pdsref2_c(2)) then
            nmtests = nmtests + 1
            call set_bit(testbits,20)
            nptests = nptests + 1
            ngtests(3) = ngtests(3) + 1
          end if
          call conf_test(masv88,pdsref2_c(1),pdsref2_c(3),pdsref2_c(4),   &
                         pdsref2_c(2),1,c3)
          cmin3 = min(cmin3,c3)
        end if
...
!       1.38um test
        if (.not. hi_elev) then
          if (nint(masv188) .ne. nint(bad_data)) then
            ...
            if (masv188 .le. pdsref3_c(2)) then
              nmtests = nmtests + 1
              call set_bit(testbits,16)
              nptests = nptests + 1
              ngtests(3) = ngtests(3) + 1
            end if
            call conf_test(masv188,pdsref3_c(1),pdsref3_c(3),pdsref3_c(4),   &
                           pdsref3_c(2),1,c3)
            cmin3 = min(cmin3,c3)
          end if
        end if
...
!     Thin cirrus bit check
      if (visusd .and. (.not. hi_elev) ) then
        if (nint(masv188) .ne. nint(bad_data)) then
          ...
          if (masv188 .lt. pdstci_c(1) .and. masv188 .ge. pdstci_c(2)) then
